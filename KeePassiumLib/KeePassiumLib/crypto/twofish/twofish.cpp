...
static int test_platform() {
    ...
    if( (GET32( buf ) != 0x78563412UL) || (GET32(buf+1) != 0x9a785634UL) 
        || (GET32( buf+2 ) != 0xbc9a7856UL) || (GET32(buf+3) != 0xdebc9a78UL) ) {
            return TWOFISH_ERROR_PLATFORM_GET32_IMPLEMENTED_IMPROPERLY;
    ...
    if( GET32( buf ) != 0x69029c36UL ) {
        return TWOFISH_ERROR_PLATFORM_PUT32_IMPLEMENTED_IMPROPERLY;
    ...
    for( i=1; i<32; i++ )  {
         ...
         */
        if( (x & 0xffffffffUL) != 0 ) {
            return TWOFISH_ERROR_PLATFORM_ROL_ROR_IMPLEMENTED_IMPROPERLY;
    ...
    if( BSWAP(C) != 0x12345678UL ) {
         ...
         */
        return TWOFISH_ERROR_PLATFORM_BSWAP_UNDEFINED;
    ...
    if( (b0(C)!=0x12) || (b1(C) != 0x34) || (b2(C) != 0x56) || (b3(C) != 0x78) ) {
         ...
         */
        return TWOFISH_ERROR_PLATFORM_SELECT_BYTE_TEST_IMPLEMENTED_IMPROPERLY;
        // Twofish_fatal( "Twofish code: SELECT_BYTE not implemented properly" );
    }
    return TWOFISH_SUCCESS;
}
...
static int test_vector( Twofish_Byte key[], int key_len, Twofish_Byte p[16], Twofish_Byte c[16] ) {
    ...
    /* Prepare the key */
    if (int err = Twofish_prepare_key( key, key_len, &xkey )) return err;
    ...
    for( i=0; i<2; i++ ) {
        ...
        if( memcmp( c, tmp, 16 ) != 0 ) {
            return TWOFISH_ERROR_TEST_ENCRYPTION_FAIL;
            // Twofish_fatal( "Twofish test encryption failure" );
        }

        /* Decrypt and test */
        Twofish_decrypt( &xkey, c, tmp );
        if( memcmp( p, tmp, 16 ) != 0 ) {
            return TWOFISH_ERROR_TEST_DECRYPTION_FAIL;
            // Twofish_fatal( "Twofish test decryption failure" );
        }
    }
...
static int test_vectors() {
    ...
    if (int err = test_vector( k128, 16, p128, c128 )) return err;
    if (int err = test_vector( k192, 24, p192, c192 )) return err;
    if (int err = test_vector( k256, 32, p256, c256 )) return err;
...
static int test_sequence( int key_len, Twofish_Byte final_value[] ) {
    ...
    for( i=1; i<50; i++ ) {
         ...
         */
        if (int err = Twofish_prepare_key( p+16, key_len, &xkey )) return err;
        ...
        if( memcmp( tmp, p, 16 ) != 0 ) {
            return TWOFISH_ERROR_TEST_SEQUENCE_DECRYPTION;
    ...
    if( memcmp( p, final_value, 16 ) != 0 )  {
        return TWOFISH_ERROR_TEST_SEQUENCE_ENCRYPTION;
...
static int test_sequences() {
    ...
    if (int err = test_sequence( 16, r128 )) return err;
    if (int err = test_sequence( 24, r192 )) return err;
    if (int err = test_sequence( 32, r256 )) return err;
...
static int test_odd_sized_keys() {
     ...
     */
    memset( buf, 0, sizeof( buf ) );
    if (int err = Twofish_prepare_key( buf, 16, &xkey )) return err;
    ...
    for( i=31; i>=0; i-- ) {
        ...
        if (int err = Twofish_prepare_key( buf, i, &xkey )) return err;

        /* Expand the corresponding padded key of regular length */
        int err = Twofish_prepare_key( buf, i<=16 ? 16 : (i<= 24 ? 24 : 32), &xkey_two );
        if (err) return err;

        /* Compare the two */
        if( memcmp( &xkey, &xkey_two, sizeof( xkey ) ) != 0 ) {
            return TWOFISH_ERROR_TEST_ODD_SIZED_KEYS;
...
static int self_test() {
    ...
    if (int err = test_vectors()) return err;

    /* 
     * If at all possible you should run these tests too. They take
     * more time, but provide a more thorough coverage.
     */
    if (int err = test_sequences()) return err;

    /* Test the odd-sized keys. */
    if (int err = test_odd_sized_keys()) return err;
...
static int fill_keyed_sboxes( Twofish_Byte S[], int kCycles, Twofish_key * xkey ) {
    ...
    default: 
        /* This is always a coding error, which is fatal. */
        return TWOFISH_ERROR_FILL_KEYED_SBOXES;
...
int Twofish_initialise() {
    ...
    if (int err = test_platform()) { return err; }
     ...
     */
    if (int err = self_test()) return err;
...
int Twofish_prepare_key( Twofish_Byte key[], int key_len, Twofish_key * xkey )
    ...
    if( Twofish_initialised == 0 ) {
         ...
         */
        return TWOFISH_ERROR_NOT_INITIALIZED;
    ...
    if( key_len < 0 || key_len > 32 ) {
         ...
         * on the key size. 
         */
        return TWOFISH_ERROR_ILLEGAL_KEY_LENGTH;
    ...
    if (int err = fill_keyed_sboxes( &K[32], kCycles, xkey )) return err;
